...
!--
  ...
  use M_T_MixModel, only: MixModel_Param_Update,MixModel_XPoleToXSite
  use M_T_MixPhase, only: T_MixPhase, MixPhase_NormCompo
  ...
  LoopTP: do jTP=1,N
    ...
    call MixModel_Param_Update(TdgK,Pbar,S)
    !-----------------/
    !
    ...
    LoopX: do iX=1,nPt !increment 0.01 is for nPt=99 !!! 
      ...
      if(S%Model==Mix_Site) then
        ...
        !do I=1,S%NAtom; write(fTrc,'(F12.3,A1)',advance="no") P%vXAtom(I),T_; end do
        !write(fTrc,*)
      end if
      !
      !
...
subroutine WriteResults(S)
  ...
  if(S%Model==Mix_Site) then
    ...
    do I=1,S%NAtom; write(fAtom,'(A,A1)',advance="no") S%vNamAtom(I),T_; end do
    write(fAtom,*)
  end if
  !
  ...
  LoopX_: do iX=1,nPt
    ...
    !
    if(S%Model==Mix_Site) then
      do I=1,S%NAtom; write(fAtom,'(F12.3,A1)',advance="no") tAtom(iX,I),T_; end do
      write(fAtom,*)
    end if
  end do LoopX_
  !
