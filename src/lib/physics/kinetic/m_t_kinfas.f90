...
!--
  ...
  !container for variable parameters of a kinetic phase
    ...
    real(dp) :: PhiM !volume fraction of mineral IN ROCK (input), 
    !                !and then IN BOX (runtime)
    real(dp) :: QsK  !current value of saturation index
    character:: cSat !-> saturation state
    !                !cSatur=D(ISSOLU,P(RECIPI,M(INIMAL,I(NERT
  end type T_KinFas_Dat
  !
  type:: T_KinFas
  ! container for a kinetic phase (generally minerals)
  ! with kinetics and texture data
...
!--
  ...
  real(dp):: VolKg
  !
  !print *,"vFas(M%iFas)%NamFs=",vFas(M%iFas)%NamFs
  !print *,"vFas(M%iFas)%VolM3=",vFas(M%iFas)%VolM3
  !print *,"vFas(M%iFas)%WeitKg=",vFas(M%iFas)%WeitKg
  !print *,"M%Dat%Radius",M%Dat%Radius
  !print *,"M%Dat%SurfKg",M%Dat%SurfKg
  !call pause_
  !
  VolKg= vFas(M%iFas)%VolM3 /vFas(M%iFas)%WeitKg
  !
  !Sgrain=4pi.r2 ; Vgrain=4/3pi.r3
  if(M%Dat%Radius>Zero)  M%Dat%SurfKg= 3.0D0 *VolKg /M%Dat%Radius
  !
  if(M%Dat%SurfKg>Zero)  M%Dat%Radius= 3.0D0 *VolKg /M%Dat%SurfKg
  !
