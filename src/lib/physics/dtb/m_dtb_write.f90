...
subroutine DtbAquHkf_Tabulate(vTPCond)
  ...
  do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") Alfa(jTP), T_;   end do
  write(f1,*)
  write(f1,'(4(A,A1))', advance="no") "AQU",T_,"NUM",T_,"NAME",T_,"Beta",T_
  do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") Beta(jTP), T_;   end do
  write(f1,*)
  write(f1,'(4(A,A1))', advance="no") "AQU",T_,"NUM",T_,"NAME",T_,"dAlfdT",T_
  do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") dAlfdT(jTP), T_;   end do
  write(f1,*)
  ...
  do iSp=1,size(vDtbAquHkf)
    ...
    ! do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") tS(iSp,jTP)%Vs, T_;  end do
    ! write(f1,*)
    write(f1,'(4(A,A1))', advance="no") "AQU",T_,M%Num,T_,M%Name,T_,"Vr",T_
    do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") tS(iSp,jTP)%V0, T_;   end do
    write(f1,*)
    write(f1,'(4(A,A1))', advance="no") "AQU",T_,M%Num,T_,M%Name,T_,"Cp",T_
    do jTP=1,NN; write(f1,'(G15.8,A1)',advance="no") tS(iSp,jTP)%Cp0, T_;   end do
    write(f1,*)
    
    !-- enthalpies
    write(f2,'(4(A,A1))', advance="no") "AQU",T_,M%Num,T_,M%Name,T_,"Hr",T_
    do jTP=1,NN; write(f2,'(G15.8,A1)',advance="no") tS(iSp,jTP)%H0/1.0D3, T_;   end do
    write(f2,*)
    !
    !-- Gibbs/RT/ln10
    ...
    !!!  do jTP=1,NN; write(f4,'(G15.8,A1)',advance="no") tS(iSp,jTP)%AquSize, T_; end do
    !!!  write(f4,*)
    !
    !!!  !write logK's
    ...
    !!!  do jTP=1,NN; write(fLogK,'(G15.8,A1)',advance="no") tS(iSp,jTP)%logK, T_; end do
    !!!  write(fLogK,*)
    !
  end do
  !---------------------------------------------/tabulate aqu'species --
...
subroutine DtbAquThr_Tabulate(vTPCond)
  ...
  real(dp):: H,S,dum ! for compatibility
  !
  real(dp):: Rho_ref,Alfa_ref,dAlfdT_ref
  ...
  do jTP=1,NN
    ...
    call EosFluid_Calc("H2O","HAAR",TdgK,Pbar,G_H2O,H,S,V_H2O,dum,Ok)
    !! H,S not computed here !!
...
!--
  ...
  !do I=1,nEleDtb; write(f,'(G15.8,A1)',advance="no") vS0Ele(I),T_; end do; write(f,*)
