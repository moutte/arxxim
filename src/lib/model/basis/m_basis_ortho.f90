...
subroutine Matrix_Sho(T)
  ...
  do I=1,nC
    !do J=1,nS
      ...
      write(F,'(*(F7.3,A1))') (T(I,J), T_, J=1,nS)
    !end do
    write(F,*)
  end do
  !
...
subroutine Basis_Ortho_Compute_Debug(vIndexBuffer,tAlfSp,tXOrthoBasis)
  ...
  if (idebug>1) then
    do iV = 1, nE
      if (iV<=nV) &
      & write(F,'(A,I3,A)',advance="no") 'Buffer Basis       ',iV,   ' = '
      if (iV>nV)  &
      & write(F,'(A,I3,A)',advance="no") 'Orthogonal Formula ',iV-nV,' = '
      do iE = 1, nE
        coef = tOrthoBasis(iV, iE)
        !! if ( abs(coef) > 0.00001) write(F,'(A4,A,F8.3,A)',advance="no") &
        !! & trim( vEle(iCpnEle(iE))%NamEl),'(',coef, ') '
        if ( abs(coef) > 0.00001) &
        write(F,'(I3,A,F8.3,A)',advance="no") iE,'(',coef, ') '
      end do
