module M_T_Species
  ...
  ! container for storing thermo properties of a species at current (T,P)
  ! for future use ??
  ...
  type:: T_Species !container for storing a SPECIES' structure
    ...
    ! case iDiscret/= 0 :
    !   = species generated by discretization of a mixture
    ...
    !
    !G0rt is G/RT of formation = deltaG's of ASSOCIATION/PRECIPITATION
    !caveat: 
  ...
  ! --> NO, not good idea ...
  ! currently, the phase is associated through the iPhase index  --> NO !!
  !
  ...
  !
  ! NB1: 
  ! what is referred to as "Gibbs energies" is actually G/RT, i.e. -LnK
  !
  ! NB2: 
  ! if the database is in "logK", it is generally log10, then LnK=Ln10*logK
  ! 
  ! deltaG's for sec'species and minerals are for formation reactions:
  !   Prim'Species -> Second'Species
  !   DG0(iSc)=  vSpc(iSc)%G0 
  !           - dot_product(tStNu(1:nCp,iSc),vSpc(1:nCp)%G0)
  !
  ! each time basis is changed, DG0 must be re-calculated,
  ! thus, DG0 not stored in vSpc
  !
  ! TODO:
  ! currently, in speciation calcul'n, MIN species are used where the 
  ! corresponding PURE phase should be used ...
  !
  logical,parameter:: Use_Synonym= .true.
  
contains

subroutine Species_Append(vSpcAdd,vSpc)
!--
!-- Append vSpcAdd to vSpc
!-- -> produce a new vSpc
!--
  type(T_Species),intent(in):: vSpcAdd(:)
  type(T_Species),allocatable,intent(inout):: vSpc(:)
  !
...
subroutine Species_Rename(sName)
  ...
  if(trim(sName)=="H[+]")   sName= "H+"
  if(trim(sName)=="OH[-]")  sName= "OH-"
  if(trim(sName)=="O2(AQ)") sName= "O2_AQ"
  if(trim(sName)=="O2,AQ")  sName= "O2_AQ"
  !
  if(Use_Synonym) return
  !
  !--- replace ",AQ" and "(AQ)" by "_AQ"
  S= ",AQ"
