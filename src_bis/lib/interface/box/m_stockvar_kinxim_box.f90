module M_stockvar_KINXIM_BOX

  use m_kinds,only: dp
  use m_trace
  use M_TimeMng, only : ERRORTimeStepTooSmall
...
contains 
  ...
  subroutine set_stockvar(X, V, T, DT, EPSILON, TAG) 
    ...
    real(dp), intent(in) :: T, DT, V 
    real(dp), intent(in) :: EPSILON 
    character, intent(in) :: TAG 
 
    call Info_('Set_Stockvar')
    !write(*,*) ' I Set stockvar', X, V, T, DT, EPSILON, TAG  
    ...
    if (index_stockvar < MAX_NBTIMESTEP) then 
       ...
       if (.not.(warnout)) then 
          ...
          write(*,*) "===============================================" 
          warnout=.true.
          call FATAL_("NBTIMESTEP > MAX_NBTIMESTEP")
          !lt stop "GEOCHIMIE : NBTIMESTEP > MAX_NBTIMESTEP" 
       end if
    end if
  ...
  subroutine computemean_stockvar(X,V, T,DT, EPSILON) 
    ...
    !--- 
    call Info_('ComputeMean_Stockvar')
...
!!$          write(*,*) '----------------------------'
!!$          write(*,*) 'Looking data', Nstep 
!!$          write(*,*) 'Total V', V 
!!$          write(*,*) 'Total T', T 
!!$          write(*,*) 'Total EPSILON', EPSILON 
!!$          write(*,*) 'Total X', X
...
!!$    write(*,*) '----------------------------'
!!$    write(*,*) 'Bilan data', Nstep 
!!$    write(*,*) 'Mean V', V 
!!$    write(*,*) 'Total T', T 
!!$    write(*,*) 'Mean EPSILON', EPSILON 
!!$    write(*,*) 'Mean X', X 
!!$    write(*,*) '--------------------' 
...
end module M_stockvar_KINXIM_BOX
